...
export async function generateToolConfigs(projectPath: string, config: MonorepoConfig): Promise<void> {
  // Generate .gitignore
  const gitignoreContent = generateGitignore();
  await writeFile(path.posix.join(projectPath, '.gitignore'), gitignoreContent);

  // Generate .eslintrc.js
  if (config.tools.some(tool => tool.name === 'eslint' && tool.enabled)) {
    const eslintContent = generateEslintConfig();
    await writeFile(path.posix.join(projectPath, '.eslintrc.js'), eslintContent);
  }

  // Generate .prettierrc
  if (config.tools.some(tool => tool.name === 'prettier' && tool.enabled)) {
    const prettierContent = generatePrettierConfig();
    await writeFile(path.posix.join(projectPath, '.prettierrc'), prettierContent);
  }
...
pnpm lint-staged
`;
  await writeFile(path.posix.join(projectPath, '.husky', 'pre-commit'), preCommitContent);
...
`;
  await writeFile(path.posix.join(projectPath, '.husky', 'commit-msg'), commitMsgContent);
  ...
  await writeFile(path.posix.join(projectPath, '.lintstagedrc.json'), lintStagedContent);
...
`;
  await writeFile(path.posix.join(projectPath, 'commitlint.config.js'), commitlintContent);
}

async function generateChangesetsConfig(projectPath: string, config: MonorepoConfig): Promise<void> {
  ...
  await writeFile(path.posix.join(projectPath, '.changeset', 'config.json'), changesetConfig);
...
`;
  await writeFile(path.posix.join(projectPath, '.changeset', 'README.md'), changesetReadme);
}
...
`;
  await writeFile(path.posix.join(projectPath, '.env.example'), envExample);
...
LOG_LEVEL=debug
`;
  await writeFile(path.posix.join(projectPath, '.env.local'), envLocal);
}

async function generateAdditionalToolConfigs(projectPath: string, config: MonorepoConfig): Promise<void> {
  ...
  await writeFile(path.posix.join(projectPath, '.vscode', 'settings.json'), vscodeSettings);
  ...
  await writeFile(path.posix.join(projectPath, '.vscode', 'extensions.json'), vscodeExtensions);
  ...
  await writeFile(path.posix.join(projectPath, '.vscode', 'launch.json'), launchConfig);
}